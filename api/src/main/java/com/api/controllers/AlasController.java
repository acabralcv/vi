...
public class AlasController {
    ...
    public ResponseEntity actionDetails(@RequestParam(name = "id") UUID id) {
        ...
        try {
            ...
            return ResponseEntity.ok().body(new BaseResponse().getObjResponse(oAla != null ? 1 : 0, "ok", oAla.get()));
     ...
     * @return
     */
    @RequestMapping(value = {"api/alas/delete"}, method = {RequestMethod.POST})
    public ResponseEntity actionDelete(@RequestBody Ala objAla) {

        try{

            Ala oAla = alaRepository.findById(objAla.getId())
                    .orElseThrow(() -> new Exception(Ala.class.getName() + " not found with id '" + objAla.getId().toString() + "' on actionDelete"));

            oAla.setDateUpdated(UtilsDate.getDateTime());
            oAla.setStatus(Helper.STATUS_DISABLED);
            alaRepository.save(oAla);

            return ResponseEntity.ok().body(new BaseResponse().getObjResponse(1,"ok", oAla));
