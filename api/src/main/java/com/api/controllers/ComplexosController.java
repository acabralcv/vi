...
public class ComplexosController {
     ...
     * @return
     */
    @RequestMapping(value = {"api/complexos/delete"}, method = {RequestMethod.POST})
    public ResponseEntity actionDelete(@RequestBody Complexo objComplexo) {

        try{

            Complexo oComplexo = complexoRepository.findById(objComplexo.getId())
                    .orElseThrow(() -> new Exception(Complexo.class.getName() + " not found with id '" + objComplexo.getId().toString() + "' on actionDelete"));

            oComplexo.setDateUpdated(UtilsDate.getDateTime());
            oComplexo.setStatus(Helper.STATUS_DISABLED);
           complexoRepository.save(oComplexo);

            return ResponseEntity.ok().body(new BaseResponse().getObjResponse(1,"ok", objComplexo));
